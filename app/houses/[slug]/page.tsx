...
export default async function HousePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="house-page" style={{ "--house-a": house.colors[0], "--house-b": house.colors[1], "--house-c": house.colors[2] } as React.CSSProperties}>
    ...
    <section className="house-dossier"><aside><div className="dossier-banner"><HouseArtwork house={house} kind="banner"/></div><p className="eyebrow">Known titles</p><ul><li>{house.primaryTitle}</li>{house.titles.map(t => <li key={t}>{t}</li>)}</ul>{house.relics.length > 0 && <><p className="eyebrow aside-gap">Relic{house.relics.length > 1 ? "s" : ""}</p><ul>{house.relics.map(r => <li key={r}>{r}</li>)}</ul></>}</aside><article><p className="eyebrow">An account of the House</p><p className="dropcap">{house.character}</p>{house.lore.map(p => <p key={p}>{p}</p>)}<blockquote>{house.motto}</blockquote>{house.saying && <p className="league-saying">The Council says: “{house.saying}”</p>}</article></section>
